...
function getPowerFunction(exponent) {
  return function funcResult(arg) {
    return arg ** exponent;
  };
}
...
function getPolynom(...arr) {
  const newArr = arr;
  return function polyFunc(arg) {
    return newArr.reduce(
      (b, a, index) => arg ** (newArr.length - index - 1) * a + b, 0,
    );
  };
}
...
function logger(func, logFunc) {
  return function wrapp(...params) {
    const param = JSON.stringify(params);
    const unfold = param.slice(1, param.length - 1);
    const body = `${func.name}(${unfold})`;
    logFunc(`${body} starts`);
